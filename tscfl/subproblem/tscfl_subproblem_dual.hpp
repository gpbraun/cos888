...
public:
    explicit SubproblemDual(const TSCFLInstance &inst_)
        : Subproblem(inst_),
          ...
          model(env),
          cplex(env),
          l1(env, inst_.nI, 0.0, IloInfinity, ILOFLOAT),
          l2(env, inst_.nJ, 0.0, IloInfinity, ILOFLOAT),
    ...
    void build_base_model()
    {
        ...
        for (int i = 0; i < inst.nI; ++i)
            for (int j = 0; j < inst.nJ; ++j)
                model.add(m1[j] - l1[i] <= inst.c[i][j]);

        for (int j = 0; j < inst.nJ; ++j)
            for (int k = 0; k < inst.nK; ++k)
                model.add(m2[k] - m1[j] - l2[j] <= inst.d[j][k]);
...
public:
    ...
    void solve(const IloNumArray &a_vals, const IloNumArray &b_vals) override
    {
        ...
        // 2) Resolve o LP dual
        if (!cplex.solve())
            throw std::runtime_error("SubproblemDual: CPLEX failed to solve dual subproblem.");
