...
LRPBalance::LRPBalance(const TSCFLInstance& inst_)
    : LRP(inst_), m1(env, inst_.nK), m2(env, inst_.nJ), g1(env, inst_.nK), g2(env, inst_.nJ) {
    fill_zero(m1);
    fill_zero(m2);
    fill_zero(g1);
    fill_zero(g2);
...
IloNum LRPBalance::norm2sq() const {
...
void LRPBalance::update_multipliers(IloNum step) {
    if (step <= 0.0) return;
