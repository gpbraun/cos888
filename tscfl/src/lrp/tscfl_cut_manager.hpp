...
class Cut {
   public:
    ...
    virtual IloNum compute_lhs(const TSCFLInstance& inst, const IloNumMatrix& x_lr,
                               const IloNumMatrix& y_lr, const IloNumArray& a_lr,
                               const IloNumArray& b_lr) const = 0;

    // Contribuição do corte para os custos agregados
    virtual void add_to_costs(const TSCFLInstance& inst, IloNumArray& cost_a, IloNumArray& cost_b,
                              IloNumMatrix& cost_x, IloNumMatrix& cost_y) const = 0;
};
...
class FlowCoverCut : public Cut {
   public:
    enum class NodeType { PLANT, DEPOT };
   ...
   public:
    ...
    const IloNumArray& cost() const noexcept { return cost_; }

    IloNum compute_lhs(const TSCFLInstance& inst, const IloNumMatrix& x_lr,
                       const IloNumMatrix& y_lr, const IloNumArray& a_lr,
                       const IloNumArray& b_lr) const override;

    void add_to_costs(const TSCFLInstance& inst, IloNumArray& cost_a, IloNumArray& cost_b,
                      IloNumMatrix& cost_x, IloNumMatrix& cost_y) const override;
};
...
class SubsetRowCut : public Cut {
   ...
   public:
    ...
    const IloNumArray& coeff() const noexcept { return coeff_; }
...
class CutManager {
   ...
   public:
    ...
    std::vector<std::unique_ptr<Cut>>& data() { return cuts; }
    const std::vector<std::unique_ptr<Cut>>& data() const { return cuts; }
    ...
    bool add_flow_cover(FlowCoverCut::NodeType node_type, int index, const IloNumArray& cost,
                        IloNum rhs);
    ...
    void update_status(const IloNumMatrix& x_lr, const IloNumMatrix& y_lr, const IloNumArray& a_lr,
                       const IloNumArray& b_lr, int extra_age);
};

// Alias para manter compatibilidade com nome antigo
using FlowCoverCutSet = CutManager;
