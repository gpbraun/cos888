...
std::size_t FlowCoverCut::compute_hash(NodeType node_type, int idx, const IloNumArray& cost) {
    std::string key;
...
IloNum FlowCoverCut::compute_lhs(const TSCFLInstance& inst, const IloNumMatrix& x_lr,
...
void FlowCoverCut::add_to_costs(const TSCFLInstance& inst, IloNumArray& cost_a, IloNumArray& cost_b,
...
std::size_t SubsetRowCut::compute_hash(Family family, const IloNumArray& coeff) {
    std::string key;
    key.reserve(32 + 4 * static_cast<std::size_t>(coeff.getSize()));
...
IloNum SubsetRowCut::compute_lhs(const TSCFLInstance&, const IloNumMatrix&, const IloNumMatrix&,
...
void SubsetRowCut::add_to_costs(const TSCFLInstance&, IloNumArray& cost_a, IloNumArray& cost_b,
...
bool CutManager::add(std::unique_ptr<Cut> cut) {
    auto res = hashes.insert(cut->hash);
    if (!res.second) return false;  // corte duplicado
    cuts.push_back(std::move(cut));
    return true;
}

// Conveniências para FlowCover
bool CutManager::add_flow_cover(const FlowCoverCut& cut) {
    auto ptr = std::make_unique<FlowCoverCut>(cut);
    return add(std::move(ptr));
}

bool CutManager::add_flow_cover(FlowCoverCut::NodeType node_type, int index,
                                const IloNumArray& cost, IloNum rhs) {
    auto ptr = std::make_unique<FlowCoverCut>(node_type, index, cost, rhs);
    return add(std::move(ptr));
}

// Conveniências para SubsetRow
bool CutManager::add_subset_row(const SubsetRowCut& cut) {
    auto ptr = std::make_unique<SubsetRowCut>(cut);
    return add(std::move(ptr));
}

bool CutManager::add_subset_row(SubsetRowCut::Family family, const IloNumArray& coeff, IloNum rhs) {
    auto ptr = std::make_unique<SubsetRowCut>(family, coeff, rhs);
    return add(std::move(ptr));
}

int CutManager::count(Cut::Status s) const {
    int cnt = 0;
...
IloNum CutManager::norm2sq() const {
    IloNum s = 0.0;
    for (const auto& c : cuts) {
        if (c->status != Cut::Status::CI) s += c->overflow * c->overflow;
    }
    return s;
}

void CutManager::update_multipliers(IloNum step) {
    if (step <= 0.0) return;
...
void CutManager::update_costs() {
    ...
    for (const auto& c : cuts) {
        if (c->status == Cut::Status::CI || c->u <= EPS) continue;
        c->add_to_costs(inst, cost_a, cost_b, cost_x, cost_y);
    }
}

void CutManager::update_status(const IloNumMatrix& x_lr, const IloNumMatrix& y_lr,
