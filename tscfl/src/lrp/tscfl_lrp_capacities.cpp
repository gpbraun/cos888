...
LRPCapacity::LRPCapacity(const TSCFLInstance& inst_)
    : LRP(inst_), l1(env, inst_.nI), l2(env, inst_.nJ), g1(env, inst_.nI), g2(env, inst_.nJ) {}

// ---------------------------------------------------------------------
//  solve
// ---------------------------------------------------------------------

IloNum LRPCapacity::solve() {
    ...
    for (IloInt k = 0; k < inst.nK; ++k) {
        ...
        for (IloInt i = 0; i < inst.nI; ++i) {
            for (IloInt j = 0; j < inst.nJ; ++j) {
                IloNum cost = inst.c[i][j] + inst.d[j][k] + l1[i] + l2[j] + cuts.cost_x[i][j] +
                              cuts.cost_y[j][k];
    ...
    for (IloInt i = 0; i < inst.nI; ++i) g1[i] = IloSum(x[i]) - inst.p[i] * a[i];

    for (IloInt j = 0; j < inst.nJ; ++j) g2[j] = IloSum(y[j]) - inst.q[j] * b[j];
...
IloNum LRPCapacity::norm2sq() const {
...
void LRPCapacity::update_multipliers(IloNum step) {
    if (step <= 0.0) return;

    for (IloInt i = 0; i < inst.nI; ++i) l1[i] = IloMax(0.0, l1[i] + step * g1[i]);

    for (IloInt j = 0; j < inst.nJ; ++j) l2[j] = IloMax(0.0, l2[j] + step * g2[j]);
