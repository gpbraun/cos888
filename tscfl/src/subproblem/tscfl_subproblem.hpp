...
class Subproblem {
   ...
   public:
    enum class Mode { DUAL, PRIMAL, NET };
    ...
    IloNum solve_primal_heuristic(const IloNumArray& a_frac, const IloNumArray& b_frac,
                                  IloNumArray& a_int, IloNumArray& b_int);

    // Heurística primal 2:
    // Lê (a,b) fracionários direto do modelo (var_a, var_b) e chama a função acima.
    IloNum solve_primal_heuristic(const IloCplex& cpx, const IloNumVarArray& var_a,
