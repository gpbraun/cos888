...
TSCFLSolverSubgradient::TSCFLSolverSubgradient (const TSCFLInstance &inst_, LRP::Mode rmode,
    ...
    : TSCFLSolver (inst_), // chama construtor da base
      relaxation (LRP::create (inst_, rmode)), subproblem (Subproblem::create (inst_, smode)),
    ...
    if (log_output)
        {
            std::cout << "[SG] Iniciando Subgradiente\n\n"
                      ...
                      << "UB" << std::setw (12) << "gap" << std::setw (12) << "step"
                      << std::setw (12) << "||g||^2" << std::setw (10) << "|CA|" << std::setw (10)
                      << "|PA|" << std::setw (10) << "|CI|"
                      << "\n"
                      << std::string (130, '-') << "\n"
                      << std::defaultfloat;
