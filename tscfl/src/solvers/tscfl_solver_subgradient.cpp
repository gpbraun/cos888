...
TSCFLSolverSubgradient::TSCFLSolverSubgradient(const TSCFLInstance& inst_, LRP::Mode rmode,
...
bool TSCFLSolverSubgradient::solve(bool log_output, IloNum time_limit) {
    ...
    if (log_output) {
        std::cout << "[SG] Iniciando Subgradiente\n\n"
                  ...
                  << std::setw(12) << "gap" << std::setw(12) << "step" << std::setw(12) << "||g||^2"
                  << std::setw(10) << "|CA|" << std::setw(10) << "|PA|" << std::setw(10) << "|CI|"
                  << "\n"
                  << std::string(130, '-') << "\n"
                  << std::defaultfloat;
    }
    ...
    while (true) {
        ...
        IloNum step = 0.0;
        if (norm2 > EPS) step = IloMax(epsilon * (ub - opt_lr) / norm2, 0.0);
        ...
        if (log_output && (iter % PRINT_EVERY == 0)) {
            ...
            std::cout << std::right << std::setw(5)
                      << iter
                      // tempo
                      << std::fixed << std::setprecision(0) << std::setw(10)
                      << elapsed
                      // z_LR, LB, UB
                      ...
                      << std::fixed << std::setw(10) << nCA << std::setw(10) << nPA << std::setw(10)
                      << nCI << "\n"
                      << std::defaultfloat;
        }
    ...
    if (status == IloAlgorithm::Unknown) {
        if (ub < IloInfinity && lb > 0.0) status = IloAlgorithm::Feasible;
    }
