...
void TSCFLSolverColumnGeneration::build_initial_model() {
    // -------------------------------------------------------------
    ...
    for (IloInt k = 0; k < inst.nK; ++k) {
        while (r_remaining[k] > EPS) {
            ...
            for (IloInt i = 0; i < inst.nI; ++i) {
                if (p_remaining[i] <= EPS) continue;

                for (IloInt j = 0; j < inst.nJ; ++j) {
                    if (q_remaining[j] <= EPS) continue;
    ...
    for (IloInt k = 0; k < inst.nK; ++k) {
        for (IloInt i = 0; i < inst.nI; ++i) {
            for (IloInt j = 0; j < inst.nJ; ++j) {
                if (flow[i][j][k] > EPS) {
                    add_column_for_client(static_cast<int>(k), static_cast<int>(i),
                                          static_cast<int>(j));
                }
            }
        }
...
void TSCFLSolverColumnGeneration::add_column_for_client(int k, int i, int j) {
...
IloInt TSCFLSolverColumnGeneration::get_num_columns() const {
    IloInt total = 0;
    for (IloInt k = 0; k < inst.nK; ++k) {
        total += z[k].getSize();
    }
...
bool TSCFLSolverColumnGeneration::solve(bool log_output, IloNum time_limit) {
    auto& SP = *subproblem;
    ...
    while (true) {
        ...
        for (IloInt k = 0; k < inst.nK; ++k) {
            ...
            for (IloInt i = 0; i < inst.nI; ++i) {
                for (IloInt j = 0; j < inst.nJ; ++j) {
                    IloNum rc = rk * (inst.c[i][j] + inst.d[j][k]) - rk * (l1[i] + l2[j]) - m2[k] -
                                m1[j][k];
        ...
        if (log_output && (iter % PRINT_EVERY == 0 || !any_new)) {
            std::cout << std::right << std::setw(5) << iter << std::fixed << std::setprecision(1)
                      << std::setw(10) << elapsed << std::setprecision(0) << std::setw(15) << lb
                      << std::setw(15) << ub << std::scientific << std::setprecision(2)
                      << std::setw(12) << gap << std::fixed << std::setw(10) << get_num_columns()
                      << "\n"
                      << std::defaultfloat;
        }
