...
class RelaxationBalance : public Relaxation {
   ...
   public:
    ...
    IloNum solve() override {
        ...
        for (IloInt j = 0; j < nJ; ++j) {
            ...
            if (cost_open < cost_closed - EPS) {
                ...
                if (best_k >= 0 && best_red < 0.0 && inst.q[j] > EPS) y[j][best_k] = inst.q[j];
            } else {
                b[j] = 0.0;
                // y[j][*] já está zerado
            }
        ...
        for (IloInt k = 0; k < nK; ++k) {
            IloNum sum_y = 0.0;
            for (IloInt j = 0; j < nJ; ++j) sum_y += y[j][k];
        ...
        for (IloInt j = 0; j < nJ; ++j) {
            IloNum sum_x = 0.0;
            for (IloInt i = 0; i < nI; ++i) sum_x += x[i][j];
    ...
    void update_multipliers(IloNum step) override {
        ...
        for (IloInt k = 0; k < nK; ++k) m1[k] += step * g1[k];

        for (IloInt j = 0; j < nJ; ++j) m2[j] += step * g2[j];
