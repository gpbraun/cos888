...
class FlowCoverCut
{
  ...
  private:
    static std::size_t
    compute_hash(Type t, int idx, const IloNumArray &cost_)
    {
        std::string key;
...
class FlowCoverCutSet
{
  ...
  public:
    ...
    std::vector<FlowCoverCut> &
    data()
    {
        return cuts;
    }
    const std::vector<FlowCoverCut> &
    data() const
    {
        return cuts;
    }
    ...
    int
    count(FlowCoverCut::Status s) const
    {
        int cnt = 0;
