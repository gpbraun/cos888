...
class FlowCoverCut {
   public:
    enum Type { PLANT, DEPOT };
    enum Status { CA, PA, CI };
    ...
    FlowCoverCut(Type t, int idx, const IloNumArray& cost_, IloNum rhs_)
        : type(t), index(idx), cost(cost_), rhs(rhs_), hash(compute_hash(t, idx, cost_)) {}
...
class FlowCoverCutSet {
   ...
   public:
    ...
    double norm2sq() const {
        double s = 0.0;
        for (const auto& c : cuts) {
            if (c.status != FlowCoverCut::CI) s += c.overflow * c.overflow;
        }
        return s;
    }
    ...
    void update_multipliers(double step) {
        for (auto& c : cuts) {
            if (c.status != FlowCoverCut::CI) c.u = IloMax(0.0, c.u + step * c.overflow);
        }
    }
    ...
    void update_costs() {
        ...
        for (const auto& cut : cuts) {
            if (cut.status == FlowCoverCut::CI || cut.u <= EPS) continue;

            if (cut.type == FlowCoverCut::PLANT) {
                int i = cut.index;
                for (int j = 0; j < inst.nJ && j < cut.cost.getSize(); ++j)
                    if (IloAbs(cut.cost[j]) > EPS) cost_x[i][j] += cut.u * cut.cost[j];
            ...
            } else  // DEPOT
            {
                ...
                for (int k = 0; k < inst.nK && k < cut.cost.getSize(); ++k)
                    if (IloAbs(cut.cost[k]) > EPS) cost_y[j][k] += cut.u * cut.cost[k];
