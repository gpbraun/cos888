            ...
            for (const auto &path : instance_paths)
                {
                    ...
                    // -----------------------------------------------------------------
                    {
                        ...
                        out << "[LP]   " << std::fixed << std::setprecision(0) << solver_lp.opt
                            << "\n";

                        TSCFLSolverCplex solver_lp2(inst);
                        solver_lp2.solve(false, time_limit);

                        out << "[MP]   " << std::fixed << std::setprecision(0) << solver_lp2.lb
                            << "  " << solver_lp2.ub << "  " << solver_lp2.nodes << "  "
                            << std::fixed << std::setprecision(1) << solver_lp2.time << "\n";
                    }
                    out.flush();
                    ...
                    // -----------------------------------------------------------------
                    {
                        ...
                        out << "[MP]   " << std::fixed << std::setprecision(0) << solver_mp.lb
                            << "  " << solver_mp.ub << "  " << solver_mp.nodes << "  " << std::fixed
                            << std::setprecision(1) << solver_mp.time << "\n";
                    }
                    out.flush();
                    ...
                    // -----------------------------------------------------------------
                    {
                        ...
                        out << "[CG] " << std::fixed << std::setprecision(0) << solver_cg_net.lb
                            << "  " << solver_cg_net.ub << "  " << solver_cg_net.iter << "  "
                            << std::fixed << std::setprecision(1) << solver_cg_net.time << "\n";
                    }
                    out.flush();
                    ...
                    // -----------------------------------------------------------------
                    {
                        ...
                        out << "[RC-n] " << solver_rc_net.lb << "  " << solver_rc_net.ub << "  "
                            << solver_rc_net.iter << "  " << solver_rc_net.time << "\n";
                    }
                    out.flush();
